...
const EditEmployee = () => {
  ...
  useEffect(() => {
    const fetchEmployee = async () => {
      try {
        const response = await customFetch.get(`/employees/${id}`);
        const data = response.data;
        
        // Format ISO Date to YYYY-MM-DD for the HTML date input
        let formattedDate = '';
        if (data.joiningDate) {
          formattedDate = new Date(data.joiningDate).toISOString().substring(0, 10);
        }
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await customFetch.put(`/employees/${id}`, formData);
      toast.success('Employee profile updated successfully!');
      navigate('/employees');
    } catch (error) {
      console.error('Update employee error:', error);
      const message = error.response?.data?.message || 'Failed to update employee record';
      toast.error(message);
    } finally {
      setLoading(false);
    }
      ...
      <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-100 shadow-sm space-y-6">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            Update employee details and save the changes.
          </p>
        </div>
